...
- `standardized` 流程：`dispatch -> plan -> (kickoff | tdd -> kickoff) -> review -> verify`
...
- `/dispatch <目标>`
- `/plan <目标或task-slug>`
- 低风险车道：`/kickoff <task-slug>`
- 高风险车道：`/tdd <task-slug>` -> `/kickoff <task-slug>`
- `/review <task-slug>`
- `/verify <task-slug>`

自适应规则：

- `planning_depth=lite`：需求大体可信且边界较小时，只写一个推荐方案
- `planning_depth=full`：范围或取舍还不稳时，进入完整规划
- `execution_lane=direct`：低回归风险时跳过 TDD，直接进入实现
- `execution_lane=tdd`：公共行为变化或回归风险高时，先写失败测试
...
- `oh-imean:tdd-writer`：负责先落失败测试并确认 RED
